...
<body class="antialiased bg-slate-50 text-slate-900">
    ...
    <header class="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur-xl">
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6">
            ...
            {{-- LINHA 1: LOGO + BOTÃO MENU + SEARCH (mobile) --}}
            <div class="flex items-center justify-between gap-3 py-4 lg:hidden">

                {{-- LOGO --}}
                <a href="{{ route('home') }}" class="group shrink-0">
                    <h1 class="text-xl font-black tracking-tighter text-slate-950 sm:text-2xl">
            ...
            {{-- MENU MOBILE (escondido por defeito) --}}
            <div id="mobile-menu" class="hidden flex-col gap-4 pb-5 lg:hidden">
                ...
                <form action="{{ route('search') }}" method="GET" class="flex w-full">
                    ...
                    <div class="group/search relative flex-1">
                        ...
                        <input type="text"
                               ...
                               class="w-full rounded-l-xl border border-slate-200 bg-slate-50 py-2.5 pl-10 pr-4 text-sm outline-none focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10">

                    </div>

                    <button type="submit"
                            class="rounded-r-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-700 active:scale-95">
                ...
                <nav class="flex flex-col gap-1">
                    ...
                    <a href="{{ route('noticias') }}"
                       class="flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium transition
                              {{ request()->routeIs('noticias') ? 'bg-blue-50 font-bold text-blue-600' : 'text-slate-700 hover:bg-slate-50' }}">
                    ...
                    <a href="{{ route('categorias') }}"
                       class="flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium transition
                              {{ request()->routeIs('categorias') ? 'bg-blue-50 font-bold text-blue-600' : 'text-slate-700 hover:bg-slate-50' }}">
                    ...
                    @guest
                        ...
                        <a href="{{ route('login') }}"
                           class="flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
            ...
            {{-- DESKTOP (lg+) — LINHA ÚNICA --}}
            <div class="hidden lg:flex lg:items-center lg:justify-between lg:gap-6 lg:py-4">
                ...
                <nav class="flex items-center gap-6">
                    ...
                    <a href="{{ route('noticias') }}"
                       class="group relative inline-flex items-center gap-2 text-sm font-medium transition hover:text-blue-600
                        ...
                        <span class="absolute -bottom-1 left-0 h-0.5 w-full origin-left rounded-full bg-blue-600 transition-transform duration-300
                                    {{ request()->routeIs('noticias') ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100' }}"></span>
                    </a>
                    ...
                    <a href="{{ route('categorias') }}"
                       class="group relative inline-flex items-center gap-2 text-sm font-medium transition hover:text-blue-600
                        ...
                        <span class="absolute -bottom-1 left-0 h-0.5 w-full origin-left rounded-full bg-blue-600 transition-transform duration-300
                                    {{ request()->routeIs('categorias') ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100' }}"></span>
                    </a>
                    ...
                    @guest
                        ...
                        <a href="{{ route('login') }}"
                           class="group inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold text-slate-600 transition duration-200 hover:bg-blue-50 hover:text-blue-600">
                    ...
                    @auth
                        ...
                        <form action="{{ route('logout') }}" method="POST" class="inline">
                            ...
                            <button type="submit"
                                    class="group inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold text-slate-600 transition duration-200 hover:bg-red-50 hover:text-red-600">
                ...
                <form action="{{ route('search') }}" method="GET" class="flex shrink-0">
                    ...
                    <div class="group/search relative w-64 xl:w-72">
                        ...
                        <input type="text"
                               ...
                               value="{{ request('search') }}"
                               placeholder="Pesquisar..."
                               class="w-full rounded-l-xl border border-slate-200 bg-slate-50 py-2.5 pl-10 pr-4 text-sm outline-none transition duration-200 placeholder:text-slate-400 hover:border-slate-300 hover:bg-white focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10">

                    </div>

                    <button type="submit"
                            class="inline-flex items-center gap-2 rounded-r-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white transition duration-200 hover:bg-blue-700 active:scale-[0.98]">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                            <circle cx="11" cy="11" r="7"/>
                            <path d="m21 21-4.3-4.3"/>
                        </svg>
